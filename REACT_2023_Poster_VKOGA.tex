...
%	\footerqrcode{https://orcid.org/0000-0003-4733-9872}
	...
	\footer{
		...
		[4] G. Santin, and B. Haasdonk, in \textit{Model Order Reduction}, Vol. 1, De Gruyter \textbf{2019}
	}
...
\begin{posterboxenv}[title=1. Introduction]{name=intro,column=1,row=5,span=4,rowspan=2}
	\begin{itemize}
		...
		\item Kernel models are deterministic and fast to train\textsuperscript{[4]}
%		\item Goal oriented training increases data efficiency
%		\item Challenge: Provide accurate surrogates with few training data
		\item We extend the novel two-layered Kernel Models\textsuperscript{[3]} with a goal-oriented approach
	\end{itemize}
\end{posterboxenv}
